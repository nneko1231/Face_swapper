...
# Fungsi untuk memeriksa dan mengunduh model pra-latih yang diperlukan
def check_ckpts():
    pretrain_model_url = {
    ...
    # Mengunduh bobot model jika belum ada
...
# Fungsi untuk mengatur enhancer dengan RealESRGAN
def set_realesrgan():
...
# Fungsi untuk pemulihan wajah
def face_restoration(img, background_enhance, face_upsample, upscale, codeformer_fidelity, upsampler, codeformer_net, device):
    """Jalankan prediksi tunggal pada model"""
    try: # Blok try global untuk menangani kesalahan
        # Mengatur pengaturan default untuk demo
        has_aligned = False
        ...
        upscale = int(upscale) # Konversi tipe ke int
        if upscale > 4: # Hindari penggunaan memori berlebih karena upscale terlalu besar
            upscale = 4 
        if upscale > 2 and max(img.shape[:2])>1000: # Hindari penggunaan memori berlebih karena resolusi gambar terlalu besar
            upscale = 2 
        if max(img.shape[:2]) > 1500: # Hindari penggunaan memori berlebih karena resolusi gambar terlalu besar
            upscale = 1
        ...
        if has_aligned:
            # Input wajah sudah dipotong dan disejajarkan
            img = cv2.resize(img, (512, 512), interpolation=cv2.INTER_LINEAR)
            face_helper.is_gray = is_gray(img, threshold=5)
            face_helper.cropped_faces = [img]
        else:
            face_helper.read_image(img)
            # Mendapatkan landmark wajah untuk setiap wajah
            num_det_faces = face_helper.get_face_landmarks_5(
            only_center_face=only_center_face, resize=640, eye_dist_threshold=5
            )
            # Menyejajarkan dan memutar setiap wajah
            face_helper.align_warp_face()

        # Pemulihan wajah untuk setiap wajah yang dipotong
        ...
        # Paste_back
        if not has_aligned:
            # Upsample latar belakang
            if bg_upsampler is not None:
                # Saat ini hanya mendukung RealESRGAN untuk upsampling latar belakang
                bg_img = bg_upsampler.enhance(img, outscale=upscale)[0]
            else:
                bg_img = None
            face_helper.get_inverse_affine(None)
            # Menempelkan setiap wajah yang dipulihkan ke gambar input
